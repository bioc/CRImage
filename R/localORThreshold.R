localORThreshold <-
function(imgG,img,window,excludeWhite=FALSE){
	...
	labV=convertRGBToLAB(channelVectors)
	imgLAB=sqrt(labV[1,]^2+labV[2,]^2+labV[3,]^2)
...
#exclude white pixel from thresholding
				if(length(as.vector(imgGL[imgGL != 1]))>0){
					iccsp_rgb=Phansalkar_threshold(as.vector(imgGL[imgGL != 1]))
					iccsp_lab=Phansalkar_threshold(as.vector(imgLABL[imgLABL != 1]))
				}else{
					iccsp_rgb=1.0
					iccsp_lab=1.0
				}
