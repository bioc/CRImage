...
function(classifier=classifier,inputFolder=inputFolder,outputFolder=outputFolder,identifier=identifier,numSlides=numSlides,cancerIdentifier=cancerIdentifier,maxShape=800,minShape=40,failureRegion=2000,slideToProcess=NA,KS=TRUE,colors=c(),classesToExclude=c(),threshold="otsu",numWindows=2,classifyStructures=FALSE,ksToExclude=c(),pixelClassifier=NA,densityToExclude=c(),numDensityWindows=32,resizeFactor=4,plotCellTypeDensity=TRUE){
	...
	nameFile=strsplit(filenames[i],"\\.")[[1]][1]
		if(is.na(slideToProcess)){
			classificationError=try(classificationAperio(pathToFolder,filenames[i],pathToOutputFolderImgDir,classifier,pathToOutputFolderImgDirFiles,pathToOutputFolderImgDir,blockSlice,sliceColors,sizeO,i,cancerIdentifier,maxShape=maxShape,minShape=minShape,failureRegion=failureRegion,KS=KS,colors=colors,classesToExclude=classesToExclude,threshold=threshold,numWindows=numWindows,pathToOutputFolderImgDirStructures,classifyStructures=classifyStructures,pathToOutputFolderImgDirCells,ksToExclude=ksToExclude,pixelClassifier=pixelClassifier,densityToExclude=densityToExclude,numDensityWindows=numDensityWindows,plotCellTypeDensity=plotCellTypeDensity))
		}else{
			slice=blockSlice[as.character(blockSlice$block)==nameFile,2]
			if(slice == slideToProcess){
				classificationError=try(classificationAperio(pathToFolder,filenames[i],pathToOutputFolderImgDir,classifier,pathToOutputFolderImgDirFiles,pathToOutputFolderImgDir,blockSlice,sliceColors,sizeO,i,cancerIdentifier,maxShape=800,minShape=40,failureRegion=2000,KS=KS,colors=colors,classesToExclude=classesToExclude,threshold=threshold,numWindows=numWindows,pathToOutputFolderImgDirStructures,classifyStructures=classifyStructures,pathToOutputFolderImgDirCells,ksToExclude=ksToExclude,pixelClassifier=pixelClassifier,densityToExclude=densityToExclude,numDensityWindows=numDensityWindows,plotCellTypeDensity=plotCellTypeDensity))
			}else{
